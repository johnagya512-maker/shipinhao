...
def create_task(body: TaskCreate, bg: BackgroundTasks, db: Session = Depends(get_db)):
    ...
    # 成本预估校验
    est = cost_svc.estimate_cost(body.transcript, body.modules, 5,
    ...
    task = Task(
        ...
        transcript=body.transcript, keyword=body.keyword, title=body.title, author=body.author,
...
def estimate(body: TaskCreate, db: Session = Depends(get_db)):
    ...
    est = cost_svc.estimate_cost(body.transcript, body.modules, 5, provider_llm, provider_img)
...
async def upload_audio(task_id: str, bg: BackgroundTasks, file: UploadFile = File(...),
    ...
    audio_dir = settings.storage_dir / task_id / "audio"
...
def download_result(task_id: str, db: Session = Depends(get_db)):
    ...
    if not task:
        raise HTTPException(404, detail="任务不存在")
    draft_path = settings.storage_dir / task_id / "jianying" / f"{task_id}.json"
    video_path = settings.storage_dir / task_id / "video" / "output.mp4"
